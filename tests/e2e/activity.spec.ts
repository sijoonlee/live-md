...
test("history records each edit with author and a durable id, and outlives compaction", async ({page, baseURL}) => {
  ...
  const agent = new AgentClient({baseUrl: baseURL!, agentId: agentName, documentId: docId});
  await agent.load();
  ...
  expect(full[0].author).toBe(agentName);
